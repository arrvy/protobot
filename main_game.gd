...
func _ready() -> void:
	pin_input.input_deactivated.connect(on_input_handler)
	pin_input.input_activated.connect(on_input_handler)
	vent_top.input_activated.connect(on_vent_handler)
	vent_top.input_deactivated.connect(on_vent_handler)
func _process(delta):
...
func on_input_handler():
	interaction = pin_input.interaction_type
	print(interaction)

func on_vent_handler():
